...
def open_ov13855_camera(source: str, width: int = 1920, height: int = 1080, fps: int = 30):
    ...
    :param width: OV13855支持的分辨率（1920x1080/1280x720/640x480）
    :param height: 摄像头高度
    :param fps: 帧率（建议15-30）
    :return: cv2.VideoCapture对象
    """
    # OV13855默认节点是/dev/video11，强制使用V4L2接口
    cam_id = int(source) if source.isdigit() else 11
    ...
    if not cap.isOpened():
        # 降级尝试720P分辨率
        print(f"[WARN] 无法打开{width}x{height}分辨率，尝试1280x720...")
        cap = cv2.VideoCapture(cam_id, cv2.CAP_V4L2)
        cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'NV12'))
...
def draw_detections(
    ...
    line_thickness = max(2, int(round(0.0025 * max(frame_h, frame_w))))
    font_scale = max(0.5, line_thickness / 3.0)
    ...
    for box, conf, cls_id in zip(boxes, confs, class_ids):
        ...
        text = f"{cls_name} {float(conf):.2f}"

        cv2.rectangle(frame, (x1, y1), (x2, y2), color, line_thickness)

        (text_w, text_h), baseline = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, font_scale, font_thickness)
        # Prefer label above box; if out of frame, place inside top area.
        text_y = y1 - 6
        if text_y - text_h - baseline - 4 < 0:
            text_y = min(frame_h - baseline - 2, y1 + text_h + baseline + 4)
        bg_x1 = x1
        bg_y1 = max(0, text_y - text_h - baseline - 4)
        bg_x2 = min(frame_w - 1, x1 + text_w + 8)
        bg_y2 = min(frame_h - 1, text_y + baseline + 2)
    ...
    cv2.putText(frame, fps_text, (15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (30, 255, 255), 2)
    cv2.putText(frame, f"Behavior: {behavior_label}", (15, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 230, 0), 2)
    det_count = int(len(confs))
    max_conf = float(np.max(confs)) if det_count > 0 else 0.0
    cv2.putText(frame, f"Detections: {det_count} | MaxConf: {max_conf:.2f}", (15, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 255), 2)
    cv2.putText(frame, f"RawMaxConf: {raw_max_conf:.2f}", (15, 125), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 180, 60), 2)
    if input_mode_text:
        cv2.putText(frame, f"InputMode: {input_mode_text}", (15, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (180, 255, 120), 2)
    # 添加摄像头信息标注
    cv2.putText(frame, "Camera: OV13855 (MIPI)", (15, 175), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 140, 255), 2)
...
def parse_args():
    ...
    # OV13855默认帧率改为15（更稳定）
    parser.add_argument("--camera-fps", type=int, default=15, help="Camera FPS (OV13855: 15-30)")
...
def main():
    ...
    # 打开OV13855 MIPI摄像头（专用函数）
    cap = open_ov13855_camera(args.source, args.camera_width, args.camera_height, args.camera_fps)
    if not cap.isOpened():
        raise RuntimeError(f"Cannot open OV13855 camera (source: {args.source})")
    print(f"[INFO] OV13855 camera opened successfully (resolution: {int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))}x{int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))}, fps: {cap.get(cv2.CAP_PROP_FPS)})")
    ...
    print(f"[INFO] Source: OV13855 MIPI Camera (dev/video{args.source})")
    ...
    try:
        while True:
            ...
            # 计算FPS
            now = time.time()
            fps_inst = 1.0 / max(now - t_last, 1e-6)
            fps_avg = (frame_idx + 1) / max(now - t0, 1e-6)
            t_last = now
            fps_text = f"FPS {fps_inst:.1f} | AVG {fps_avg:.1f}"
            ...
            if reporter.enabled and (now - window_start) >= report_interval:
                ...
                if window_idx % flush_every == 0:
                    reporter.flush_spool(max_records=20)
            ...
            if not args.no_show:
                ...
                if key == ord("q"):
                    ...
                    debug_path = f"debug_frame_{frame_idx}.jpg"
        ...
        if reporter.enabled:
            reporter.flush_spool(max_records=200)
        # 释放资源
        cap.release()
        if out_writer is not None:
            out_writer.release()
